...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games
               ON teams.team_id = games.winner_id WHERE year = '2018' AND round = 'Final'")"
...
echo "$($PSQL "SELECT year, name FROM teams INNER JOIN games ON teams.team_id = games.winner_id
